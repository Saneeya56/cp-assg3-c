ASSIGNMENT NO 3  
TITLE:WAP TO CALCULATE THE NET SALARY AND SHOWING ESPENSE BY GIVING BASIC SALARIES  
STUDENT NAME:SANEEYA SHAIKH  ROLL NO :56 UIN:241PO62  TASK GIVEN BY:  
ASSIGNEE NAME:SWARALI KARKAR  
ASSIGEE ROLL NO:59  
*/  
  
  
#include<stdio.h>   int main(){  float basic,pa,hra,salary;  char d[100];  printf("\t\t***Employee salary details***\n");  start:  
printf("\nEnter Your Name:");  scanf(" %s",&d);  printf("\nEnter your basic salary:");  scanf("%f",&basic);  
//calculate the pa,hra and salary  
pa=basic*35/100;  hra=basic*40/100;  salary=basic+pa+hra;  //Displaying the details  printf("\nYour name is: %s",d);  printf("\nSalary details:%.1f",salary);   printf("\nBasic salary is:%.1f",basic);   printf("\nHouse rent Allowance is:%.1f",pa);   printf("\nPersonal expence Allowance is:%.1f",hra);  
...
Enter your basic salary:140000  
